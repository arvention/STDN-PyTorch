...
def match(threshold,
    ...
    # get best object for each anchor
    ...
    for j in range(best_anchor_i.shape[0]):
        best_object_i[best_anchor_i[j]] = j

    matched = loc_target[best_object_i]
    loc_match = encode(matched, anchors, variances)

    class_match = class_target[best_object_i] + 1
    class_match[best_object_iou < threshold] = 0
    return class_match, loc_match
